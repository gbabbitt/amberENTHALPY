...
my $pdbFrame = $mw->Frame();
	...
	my $runsFrame = $pdbFrame->Frame();
     ...
     my $boxFrame = $pdbFrame->Frame();
		my $boxLabel = $boxFrame->Label(-text=>"size of water octahedral box (e.g. 20 angstom buffer): ");
...
my $launchButton = $mw -> Button(-text => "launch 4 MD runs (ligand bound, unbound, ligand only, water only)", 
				-command => \&launch,
				-background => 'gray45',
				-foreground => 'white'
...
if($simType eq "amber"){
    ...
    sleep(2);
    system "perl MD_proteinWater.pl\n";
    sleep(2);
    print "\n\n";
...
if($simType eq "open" && $solvType eq "ex"){
    ...
    print "python MD_proteinLigand_openMM.py\n";
    print "python MD_proteinWater_openMM.py\n";
    sleep(2);
    print "\n\n";
